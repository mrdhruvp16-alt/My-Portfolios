...
// ════════════════════════════════════════════════
//  REVIEWS (marquee under "What Clients Say")
//
//  These are placeholder testimonials so the section looks finished —
//  swap them out for real client feedback whenever you have it.
//
//  HOW TO ADD/EDIT A REVIEW:
//  {
//    name: "Client Name",
//    role: "What they needed",   // e.g. "Real Estate Reel", "YouTube Channel"
//    quote: "Their words about working with you.",
//    rating: 5                    // 1–5, shown as stars
//  }
// ════════════════════════════════════════════════
const REVIEWS = [
  { name: 'Aarav Mehta',    role: 'Real Estate Reels',   quote: "Turnaround was crazy fast and the pacing made our listings actually stop the scroll. Booked him for every property we launch now.", rating: 5 },
  { name: 'Simran Kaur',    role: 'YouTube Channel',     quote: "Finally an editor who gets retention. Watch time on my videos jumped the very first upload he cut.", rating: 5 },
  { name: 'Rohan Bhatt',    role: 'Instagram Reels',     quote: "Sent raw clips, got back something that looked genuinely cinematic. Communication was easy the whole way through.", rating: 5 },
  { name: 'Priya Nair',     role: 'Podcast Clips',       quote: "He pulled the best moments from a 90-minute episode and turned them into clips that actually performed. Saved me hours every week.", rating: 4 },
  { name: 'Karan Shah',     role: 'Brand Promo Video',   quote: "Understood the brief immediately, no back-and-forth needed. The final cut sold the product better than our brief for it.", rating: 5 },
  { name: 'Ishita Verma',   role: 'Short-Form Content',  quote: "Quick, sharp edits with none of the usual delays. Exactly what I need for daily posting.", rating: 5 },
];
...
function buildReviewsMarquee() {
  ...
  const starIcon = (filled) => `<svg width="14" height="14" viewBox="0 0 24 24" fill="${filled ? 'currentColor' : 'none'}" stroke="currentColor" stroke-width="1.5"><path d="M12 2.5l2.9 6.1 6.6.8-4.9 4.6 1.3 6.6L12 17.4l-5.9 3.2 1.3-6.6L2.5 9.4l6.6-.8L12 2.5z"/></svg>`;

  const buildCard = (r) => {
    const card = document.createElement('div');
    card.className = 'review-card';
    card.innerHTML = `
      <div class="review-stars">${Array.from({ length: 5 }, (_, i) => starIcon(i < r.rating)).join('')}</div>
      <p class="review-quote">"${r.quote}"</p>
      <div class="review-person">
        <div class="review-avatar">${r.name.charAt(0)}</div>
        <div class="review-person-text">
  ...
  // Render the list twice back-to-back so the CSS animation (which slides
  // exactly -50%) loops seamlessly with no visible seam or jump.
  REVIEWS.concat(REVIEWS).forEach(r => track.appendChild(buildCard(r)));
}

let activeCat = 'shortform';

// Videos shown per carousel "page" — 2 on phones/tablets AND on regular
// laptops, 3 only on genuinely large/wide screens (>1200px).
//
// This used to switch to 3-per-page as soon as the screen crossed ~820px,
// which meant a category with exactly 3 videos filled the row exactly —
// so the prev/next buttons (and the whole nav bar) got hidden, since there
// was nothing left to page through. On phones the same category still had
// 2 pages (2 + 1) so the swipe controls worked fine there. Net effect: the
// buttons "disappeared" specifically on laptops, which looked like a bug.
//
// Raising the 3-column threshold means most laptops now behave like phones/
// tablets (2 per page) and reliably keep a 2nd page — so the prev/next
// buttons show up and actually do something — while very large/wide
// monitors still get the roomier 3-column layout.
function getPerPage() {
  return window.innerWidth > 1200 ? 3 : 2;
}
...
function buildVideoCard(item, data) {
  ...
  // For real videos, title/description/tools live as an overlay ON the
  // thumbnail itself (like a Reel/Short caption) instead of a separate text
  // block underneath — a dark scrim sits behind the text so it stays
  // readable over any footage. Empty placeholder cards keep the old
  // text-below-thumbnail layout instead, so the "add your video" instructions
  // don't overlap the centered placeholder icon.
  const caption = `
  ...
  el.innerHTML = `
    <div class="video-thumb-wrap${hasVideo ? '' : ' is-empty'}">
      ...
      ${thumbInner}
      ${hasVideo ? `<div class="video-overlay-scrim"></div><div class="video-overlay-info">${caption}</div>` : ''}
    </div>
    ${hasVideo ? '' : `<div class="video-info">${caption}</div>`}`;
